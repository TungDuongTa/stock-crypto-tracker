...
export default function SearchCommand({
  ...
  const handleSearch = async () => {
    setLoading(true);
    try {
      const results = await searchCoins(searchValue.trim());
      setCoins(results);
    } catch {
      setCoins([]);
    } finally {
      ...
      >
        <Command>
          ...
          <CommandList className="bg-gray-800! max-h-100">
            {loading ? (
              <CommandEmpty>Loading coins...</CommandEmpty>
            ) : displayCoins?.length === 0 ? (
              <div className="px-5 py-2">
                {isSearchMode ? "No result found" : "No coins available"}
              </div>
            ) : (
              <ul>
                <div className="py-2 px-4 text-sm font-medium text-gray-400 bg-gray-700 border-b border-gray-700">
                  {isSearchMode ? "Search results" : "Popular coins"}
                </div>
